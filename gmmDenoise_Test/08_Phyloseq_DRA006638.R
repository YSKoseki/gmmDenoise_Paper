...
path_seq_tab <- list(dada="./03-DADA2_DRA006638/05-Saved_object/seq_tab_dada.obj",
                     uno2="./04-UNOISE3_DRA006638/K_Saved_object/seq_tab_uno2.obj",
                     nodn="./04-UNOISE3_DRA006638/K_Saved_object/seq_tab_nodn.obj")
path_taxon_tib <- "./07-TaxonTab_DRA006638/01-Saved_object/taxon_tib.obj"
## Output directory
path_output <- "./08-Phyloseq_DRA006638"

# Construct a series of phyloseq objects (stored in one list)
## The function
construct.phyloseq <- function(path.seq.tab, path.taxon.tib, 
                               taxa.are.rows=FALSE, seq.var="seq") {
  ...
  ps <- phyloseq(seq.tab, taxon.tab)
  return(ps)
}
## Run
phylo <- pmap(
  list(path_seq_tab, path_taxon_tib),
  function(x, y) {
    construct.phyloseq(path.seq.tab=x, path.taxon.tib=y)
  }
)
