# 04_UNOISE3_DRA006638.R
# Last updated on 2022.7.14 by YK
...
path_input_fastq <- "./02-Cutadapt_DRA006638/trimmedFASTQ"
## Output
path_UNOISE3_out <- "./04-UNOISE3_DRA006638"
...
# E. Denoising with alpha=2 (the default in usearch and vsearch) and
#   abundance filtering with minsize=4 (following Tsuji et al. 2020)
list.files("D_U_max_ee/_data", full.names=TRUE) %>% 
  Denoise(strategy="unoise3", unoise_alpha=2, minsize=4, 
...
summary_tib_uno2 <- make.summary.tibble(ptrim.stats.tsv="../02-Cutadapt_DRA006638/primtrim.tsv",
...
write.csv(summary_tib_uno2, "./G_summary_tib_uno2.csv")

summary_tib_nodn <- make.summary.tibble(ptrim.stats.tsv="../02-Cutadapt_DRA006638/primtrim.tsv",
                                        merge.stats.csv="./B_merge_PE/_stats/B_sequ_length_abund.csv",
                                        lfiltd.stats.csv="./C_Minmax/_stats/C_minmax_pass.csv",
                                        qfiltd.stats.csv="./D_U_max_ee/_stats/D_max_ee_stats.csv",
...
### Run
seq_tab_uno2 <- make.seq.table("./E_Denoising/E_haplo_table.csv")
