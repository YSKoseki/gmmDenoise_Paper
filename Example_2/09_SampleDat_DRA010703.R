...
# 1. Go to the page:
#    https://www.ncbi.nlm.nih.gov/bioproject/?term=PRJDB10433
# 2. Click the 'BioSample' link on the right side of the page, which opens the page: 
#    https://www.ncbi.nlm.nih.gov/biosample?LinkName=bioproject_biosample_all&from_uid=663138
...
# Input and output
## Input: metadata.csv obtained by the shell script "01_SRAfastq_DRA010703.sh"
path_input_csv <- "./01-SRAfastq_DRA010703/metadata.csv"
## Input: biosample_result.xml downloaded at https://www.ncbi.nlm.nih.gov/bioproject/?term=PRJDB5158
path_input_xml <- "./01-SRAfastq_DRA010703/biosample_result.xml"
## Output
path_output <- "./09-SampleDat_DRA010703"
...
location <- xml_find_all(biosample, "///Attributes") %>% xml_text() %>% 
  ...
  `[`(, 2) %>% 
  str_split(r"(\d)", simplify=TRUE) %>% 
  `[`(, 1)
latlon <- xml_find_all(biosample, "///Attributes") %>% xml_text() %>%
  str_split(location, simplify=TRUE) %>%
  `[`(, 2) %>%
  str_split("Morisatoumi", simplify=TRUE) %>% 
  `[`(, 1) %>%
