# 04_UNOISE3_DRA010703.R
# Last updated on 2022.4.15 by YK
# An R script to denoise amplicon sequences using the UNOISE3 algorithm in the JAMP package
...
path_input_fastq <- "./02-Cutadapt_DRA010703/trimmedFASTQ"
## Output
path_UNOISE3_out <- "./04-UNOISE3_DRA010703"
...
# B. Merging paired-end reads with fastq_maxdiffs=0, fastq_maxdiffpct=0 and fastq_minovlen=20 
#   (so as to match with DADA2) 
Merge_PE(fastq_maxdiffs=0, fastq_maxdiffpct=0, fastq_minovlen=20)

# C. Length filtering (163-185 bps, the MiFish target region)
Minmax(min=163, max=185)

# D. Quality filtering
Max_ee(max_ee=1)

# E. Denoising with alpha=5 (the default in JAMP) and
#   abundance filtering with minsize=10 (as in the MiFish pipeline)
list.files("D_U_max_ee/_data", full.names=TRUE) %>%
  Denoise(strategy="unoise3", unoise_alpha=5, minsize=10, 
          minrelsize=0, OTUmin=0, minhaplosize=0, withinOTU=0) # No additional filtering

# F. Denoising with alpha=2 (the default in usearch and vsearch) and
#   abundance filtering with minsize=10 (as in the MiFish pipeline)
list.files("D_U_max_ee/_data", full.names=TRUE) %>% 
  Denoise(strategy="unoise3", unoise_alpha=2, minsize=10, 
          minrelsize=0, OTUmin=0, minhaplosize=0, withinOTU=0) # No additional filtering
...
### Tables of the analyses with alpha = 5 and 2
summary_tib_uno5 <- make.summary.tibble(ptrim.stats.tsv="../02-Cutadapt_DRA010703/primtrim.tsv",
...
summary_tib_uno2 <- make.summary.tibble(ptrim.stats.tsv="../02-Cutadapt_DRA010703/primtrim.tsv",
                                        merge.stats.csv="./B_merge_PE/_stats/B_sequ_length_abund.csv",
                                        lfiltd.stats.csv="./C_Minmax/_stats/C_minmax_pass.csv",
                                        qfiltd.stats.csv="./D_U_max_ee/_stats/D_max_ee_stats.csv",
                                        unoise.stats.txt="./F_Denoising/_stats/2_denoise_logs.txt",
                                        input.fastq.suff="_trimmed_R1.fastq.gz")
write.csv(summary_tib_uno5, "./H_summary_tib_uno5.csv")
write.csv(summary_tib_uno2, "./I_summary_tib_uno2.csv")
### Just a look at the denoising results with alpha = 5 and 2
alpha_5vs2_tib <- bind_cols(sample=summary_tib_uno5$sample,
                           total=summary_tib_uno5$total,
                           unoise3_a5=summary_tib_uno5$unoise,
                           unoise3_a2=summary_tib_uno2$unoise,
                           nonchim_a5=summary_tib_uno5$nonchim,
                           nonchim_a2=summary_tib_uno2$nonchim,
                           remained_a5=summary_tib_uno5$remained,
                           remained_a2=summary_tib_uno2$remained,
                           diff_remained=(summary_tib_uno5$remained-summary_tib_uno2$remained) %>% round(digits=1))
alpha_5vs2_tib
...
### Run
seq_tab_uno5 <- make.seq.table("./E_Denoising/E_haplo_table.csv")
seq_tab_uno2 <- make.seq.table("./F_Denoising/E_haplo_table.csv")
...
### Run
seqID_tib_uno5 <- make.seqID.tibble(asv.table=seq_tab_uno5, id.prefix="UNO5")
seqID_tib_uno2 <- make.seqID.tibble(asv.table=seq_tab_uno2, id.prefix="UNO2")
...
### Run
make.fasta(asv.table=seq_tab_uno5,
           header.prefix=">UNO5",
           file.name="J_ASV_seqs_uno5.fa")
make.fasta(asv.table=seq_tab_uno2,
           header.prefix=">UNO2",
           file.name="K_ASV_seqs_uno2.fa")

## Save data
## Save R objects
setwd(path_curr_wd)
path_saved_obj <- "./G_Saved_object"
dir.create(path_saved_obj, recursive=TRUE)
saveRDS(seq_tab_uno5, 
        paste0(path_saved_obj, "/seq_tab_uno5", ".obj"))
saveRDS(seq_tab_uno2, 
        paste0(path_saved_obj, "/seq_tab_uno2", ".obj"))
saveRDS(summary_tib_uno5, 
        paste0(path_saved_obj, "/summary_tib_uno5", ".obj"))
saveRDS(summary_tib_uno2, 
        paste0(path_saved_obj, "/summary_tib_uno2", ".obj"))
saveRDS(seqID_tib_uno5, 
        paste0(path_saved_obj, "/seqID_tib_uno5", ".obj"))
saveRDS(seqID_tib_uno2, 
        paste0(path_saved_obj, "/seqID_tib_uno2", ".obj"))
