...
path_input_primtrim <- "./02-Cutadapt_DRA006638/primtrim.tsv"
## Output
path_dada2_out <- "./03-DADA2_DRA006638"
...
## The error rate learning function, with parameters values set at the defaults,
##   except MAX_CONSIST set at 20 due to convergence issues
set.seed(101)
fwds_error <- fwds_filt %>% 
  learnErrors(multithread=TRUE, randomize=TRUE, verbose=TRUE, 
              MAX_CONSIST=20, nbases=1e+08)
...
##   with minOverlap = 12 and maxMismatch = 0 (defaults)
mergers <- mergePairs(fwds_dada, fwds_derep, 
                      revs_dada, revs_derep,
                      minOverlap=12, maxMismatch=0, verbose=TRUE)
...
## No Length filtering (setting 0<= x <= 1000 for convenience)
...
getSequences(seq_tab_lfiltd) %>% 
  nchar() %>% 
  table()
## Abundance filtering (had no effects as all ASVs met the condition)
seq_tab_nfiltd <- seq_tab_lfiltd
seq_tab_nfiltd[seq_tab_nfiltd<4] <- 0L
...
seq_tab_nonchim <- seq_tab_nfiltd
