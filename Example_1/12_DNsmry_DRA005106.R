...
path_input1 <- "./10-Phyloseq_DRA005106/01-Saved_object/phylo.obj"
## GMM-inferred ASV read count cutoff threshold
path_input2 <- "./11-GMMdn_DRA005106/01-Saved_object/thresh_tab.obj"
## Output directory
path_output <- "./12-DNsmry_DRA005106"
...
smry.stats <- function(phydat.pre, phydat.post) {
  ...
  fs_sp <- full_join(fs_pre, fs_post, by = "species") %>%
    arrange(desc(post))
  # Non-fish species
  nonfs_pre <- sp_pre %>% filter(class != "Actinopteri") %>%
    select(species) %>% group_by(species) %>% summarize(pre = n())
  nonfs_post <- sp_post %>% filter(class != "Actinopteri") %>%
    select(species) %>% group_by(species) %>% summarize(post = n())
